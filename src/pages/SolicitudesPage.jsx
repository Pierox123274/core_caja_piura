...
import {
  listarSolicitudes, listarNotas, agregarNota, procesarComite,
...
function fechaSolicitud(s) {
  if (s.estado === 'desembolsado' && s.desembolsado_at) return s.desembolsado_at
  if (s.decision_at) return s.decision_at
  return s.created_at
}
...
export default function SolicitudesPage() {
  ...
  const ejecutarComite = async (sol) => {
    if (!['recibido_comite', 'en_evaluacion', 'enviado'].includes(sol.estado)) return
    setProcesando(sol.id)
    setError(null)
    try {
      const r = await procesarComite(sol.id)
      setOk(`Comité: ${sol.numero_expediente} → ${r.estado}`)
      cargar()
    } catch (err) {
      setError(extractError(err))
    } finally {
              ...
              <tbody>
                {items.map((s) => (
                  <tr key={s.id}>
                    ...
                    <td>{formatDateTime(fechaSolicitud(s))}</td>
                    <td>
                      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                        {['enviado', 'recibido_comite', 'en_evaluacion'].includes(s.estado) && (
                          <button
                            ...
                            onClick={() => ejecutarComite(s)}
                          >
                            {procesando === s.id ? '…' : 'Comité'}
                          </button>
                        )}
