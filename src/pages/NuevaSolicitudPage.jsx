...
import { ArrowLeft, PlusCircle, CheckCircle2, UserPlus, Coins, Loader2 } from 'lucide-react'
...
import { useDniAutofill } from '../hooks/useDniAutofill.js'
...
export default function NuevaSolicitudPage() {
  ...
  const onDniFound = useCallback((p) => {
    setF((s) => ({ ...s, nombres: p.nombres, apellidos: p.apellidos }))
  }, [])
  const { dniLoading, dniError } = useDniAutofill(f.numero_documento, onDniFound)
  ...
  return (
    <>
      ...
      {dniError && <Alert tipo="warn">{dniError}</Alert>}

      <form onSubmit={onSubmit}>
        <Card title="Solicitante y negocio" icon={UserPlus}>
          <div className="hb-grid-2">
            <div className="hb-field">
              <label>DNI * {dniLoading && <Loader2 size={14} className="hb-spin" style={{ verticalAlign: 'middle' }} />}</label>
